...
class PagedSInventory(plugin: JavaPlugin,name: String,val row: Int): SInventory(plugin,name,row) {
    ...
    fun addPage(inv: SInventory){
        val invItems = inv.javaClass.getField("items").get(inv) as HashMap<Int,SInventoryItem>
        items.add(invItems)
    }
