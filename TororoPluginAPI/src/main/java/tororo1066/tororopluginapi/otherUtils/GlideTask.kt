...
class GlideTask(private val entity: Entity, private val travelLoc: Location, private val total: Int, private val plugin : JavaPlugin) : Runnable {
    var id = 0
    private var cur = 0
    override fun run() {
        if (cur >= total) {
    ...
    companion object{
        fun glide(player: Player, toGlide: Location, ticks: Int, plugin: JavaPlugin) {
            ...
            val task = GlideTask(player, location, ticks,plugin)
            task.id = Bukkit.getServer().scheduler.runTaskTimerAsynchronously(plugin, task, 0, 1).taskId
        }
    }
