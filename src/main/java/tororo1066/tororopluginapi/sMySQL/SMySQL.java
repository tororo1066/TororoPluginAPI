...
public class SMySQL {
    ...
    Plugin plugin;
    public SMySQL(JavaPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public ArrayList<SMySQLResult> cachedQuery(String query){
        ArrayList<SMySQLResult> result = new ArrayList<>();
        ...
        try {
            while(rs.next()){
                ...
                result.add(new SMySQLResult(localData));
            }
            rs.close();
            this.close();
            return result;
