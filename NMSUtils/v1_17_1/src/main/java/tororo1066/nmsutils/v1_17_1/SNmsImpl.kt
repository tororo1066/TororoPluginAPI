...
class SNmsImpl: SNms {
    ...
    override fun updateInventoryTitle(p: Player, inv: Inventory, title: String) {
        ...
        val packet = ClientboundOpenScreenPacket(ep.containerMenu.containerId, con, Component.nullToEmpty(null).copy().append(title))
    ...
    override fun moveRotation(p: Player, yaw: Float, pitch: Float) {
        val packet = ClientboundPlayerPositionPacket(0.0,0.0,0.0,yaw,pitch,
            ClientboundPlayerPositionPacket.RelativeArgument.values().toSet(),0,true)
